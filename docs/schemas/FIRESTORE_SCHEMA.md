...
## 📌 مجموعات الجذر (Legacy / مرجع القواعد)

القواعد في `firestore.rules` ما زالت تعرّف مسارات الجذر التالية للقراءة فقط (لا كتابة من العميل):

- `requests`, `rooms`, `roomCards` — **البيانات الفعلية** تُخزَّن وتُقرأ من مسارات الـ tenant:  
  `tenants/{tenantId}/requests`, `tenants/{tenantId}/rooms`, `tenants/{tenantId}/roomCards`.  
  مسارات الجذر قد تُستخدم في تحليلات المالك (system-wide) أو تكون قديمة؛ التطبيق يعتمد على مسارات الـ tenant.

- مجموعات أخرى تحت الجذر (مثل `guestOrders`, `notifications`, إلخ) — راجع القواعد لمعرفة المسموح به.

---

## 🌐 مسارات القراءة العامة (Phase 8 — مقصودة وموثّقة)

المسارات التالية تستخدم `allow read: if true` عن قصد. المحتوى إعدادات/صحة/ديمو فقط وليس بيانات مستخدمين أو tenant.

| المسار | الغرض | الكتابة |
|--------|--------|---------|
| `system/{settingId}` | إعدادات تطبيق عامة | معطلة (Functions فقط) |
| `systemSettings/{docId}` | إعدادات نظام، لا PII | معطلة |
| `health_check/{docId}` | فحص صحة/مراقبة | مصادق فقط |
| `emergency_alerts/{alertId}` | تنبيهات طوارئ للقراءة العامة | معطلة |
| `demoLinks/{linkId}` | روابط الديمو العامة | معطلة |

هذه ليست ثغرة؛ مراجع القواعد والتقرير المعماري تفهم أن القراءة العامة مقصودة لهذه المسارات فقط.
