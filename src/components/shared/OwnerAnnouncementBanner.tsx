 ...
 * Displays urgent announcements from owner to managers
 * Appears as scrolling ticker at the top of admin dashboards
 * Adora Hotel Management System V2
 */

import React, { useState, useEffect } from 'react';
import { X, AlertCircle, Bell, Info, Sparkles, Wrench, AlertTriangle, ChevronRight } from 'lucide-react';
import { useAuth } from '../../context/AuthContext';
import { useTenant } from '../../context/TenantContext';
import {
    subscribeToOwnerAnnouncements,
    markAnnouncementAsViewed,
    dismissAnnouncement,
    type OwnerAnnouncement
} from '../../services/ownerAnnouncementService';

export const OwnerAnnouncementBanner: React.FC = () => {
    const { user } = useAuth();
    ...
    useEffect(() => {
        if (!isManager || !tenantId || !user?.id) return;

        const unsubscribe = subscribeToOwnerAnnouncements(
            tenantId,
            branchId,
            (announcementsList) => {
        ...
        return () => unsubscribe();
    }, [isManager, tenantId, branchId, user?.id, dismissedIds, viewedIds]);
    ...
    useEffect(() => {
        const saved = localStorage.getItem(`owner_announcements_dismissed_${tenantId}`);
        if (saved) {
            try {
    ...
    const handleDismiss = async (announcementId: string) => {
        ...
        try {
            ...
            localStorage.setItem(
                `owner_announcements_dismissed_${tenantId}`,
    ...
    if (!isManager || announcements.length === 0) return null;
