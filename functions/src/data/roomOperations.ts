...
interface CheckInData {
    ...
    adults: number;
    children: number;
    expectedCheckOut?: string; // ISO date string
    needsCart?: boolean;
    notes?: string;
    tenantId: string;
}
 ...
 */
async function verifyEmployeeIdentity(
    userId: string,
    tenantId: string,
    requiredRole?: 'reception' | 'manager' | 'admin'
): Promise<{ allowed: boolean; error?: string; employeeData?: any }> {
    try {
        ...
        if (userData?.role === 'owner' || userData?.super_admin === true) {
            return { allowed: true, employeeData: userData };
        }
 ...
 * ```
 */
export const checkRoomAvailability = functions
    .region('us-central1')
    .runWith({
        timeoutSeconds: 10,
        memory: '256MB'
    })
    .https.onCall(async (data: { roomNumber: string; tenantId: string }, context): Promise<{
        available: boolean;
        roomData?: any;
        branchId?: string;
        error?: string;
        ...
        if (!employeeCheck.allowed) {
            throw new functions.https.HttpsError(
                'permission-denied',
                employeeCheck.error || 'ليس لديك صلاحية للوصول إلى هذا المستأجر'
            );
        }
 ...
 */
async function checkRoomAvailabilityHelper(
        ...
        if (!activeCardQuery.empty) {
            ...
            return {
                ...
                error: `الغرفة رقم ${roomNumber} مشغولة بالفعل من قبل: ${existingCard.guestName || 'نزيل'}\n` +
                       `تاريخ الدخول: ${existingCard.checkInTime?.toDate().toLocaleDateString('ar-SA') || 'غير محدد'}`
            };
        }
...
export const processCheckIn = functions
    ...
    .https.onCall(async (data: {
        ...
        tenantId: string;
    }, context): Promise<{ success: boolean; roomCardId?: string; error?: string }> => {
        // 1. ✅ Security: Verify authentication
        if (!context.auth) {
            throw new functions.https.HttpsError(
                'unauthenticated',
        ...
        // 2. ✅ Validate input
        if (!roomNumber || !guestData?.guestName || !tenantId) {
            throw new functions.https.HttpsError(
                'invalid-argument',
                'بيانات ناقصة: roomNumber, guestData.guestName, tenantId مطلوبة'
            );
        }
        ...
        if (guestData.adults < 1 || guestData.adults > 10) {
            throw new functions.https.HttpsError(
                'invalid-argument',
                'عدد البالغين يجب أن يكون بين 1 و 10'
            );
        }
        ...
        if (!employeeCheck.allowed) {
            throw new functions.https.HttpsError(
                'permission-denied',
                employeeCheck.error || 'ليس لديك صلاحية لهذه العملية'
            );
        }
        ...
        try {
            return await db.runTransaction(async (transaction) => {
                ...
                if (roomQuery.empty) {
                    throw new functions.https.HttpsError(
                        'not-found',
                        `الغرفة رقم ${roomNumber} غير موجودة في النظام`
                    );
                }
                ...
                if (!roomSnap.exists) {
                    throw new functions.https.HttpsError(
                        'not-found',
                        `الغرفة رقم ${roomNumber} غير موجودة`
                    );
                }
                ...
                if (currentRoomData.status === 'occupied') {
                    throw new functions.https.HttpsError(
                        'failed-precondition',
                        `الغرفة رقم ${roomNumber} مشغولة بالفعل`
                    );
                }
                ...
                if (currentRoomData.status === 'maintenance') {
                    throw new functions.https.HttpsError(
                        'failed-precondition',
                        `الغرفة رقم ${roomNumber} تحت الصيانة ولا يمكن حجزها`
                    );
                }
                ...
                if (currentRoomData.status === 'dirty' || currentRoomData.status === 'cleaning') {
                    throw new functions.https.HttpsError(
                        'failed-precondition',
                        `الغرفة رقم ${roomNumber} قيد التنظيف. يرجى الانتظار حتى تصبح جاهزة`
                    );
                }
                ...
                if (!activeCardQuery.empty) {
                    ...
                    throw new functions.https.HttpsError(
                        'failed-precondition',
                        `الغرفة رقم ${roomNumber} مشغولة بالفعل من قبل: ${existingCard.guestName || 'نزيل'}`
                    );
                }
                ...
                transaction.set(auditLogRef, {
                    ...
                    employeeName: employeeCheck.employeeData?.name || 'Unknown'
                });
...
export const checkInGuest = functions
    ...
    .https.onCall(async (data: CheckInData, context): Promise<{ success: boolean; roomCardId?: string; error?: string }> => {
        // 1. Security: Verify authentication
        if (!context.auth) {
            throw new functions.https.HttpsError(
                'unauthenticated',
        ...
        if (!roomNumber || !guestName || !tenantId) {
            throw new functions.https.HttpsError(
                'invalid-argument',
                'بيانات ناقصة: roomNumber, guestName, tenantId مطلوبة'
            );
        }
        ...
        if (!availabilityCheck.available) {
            throw new functions.https.HttpsError(
                'failed-precondition',
                availabilityCheck.error || 'الغرفة غير متاحة'
            );
        }
 ...
 * ```
 */
export const checkOutGuest = functions
    .region('us-central1')
    .runWith({
        timeoutSeconds: 30,
        memory: '256MB'
    })
    .https.onCall(async (data: CheckOutData, context): Promise<{ success: boolean; error?: string }> => {
        // 1. Security: Verify authentication
        if (!context.auth) {
