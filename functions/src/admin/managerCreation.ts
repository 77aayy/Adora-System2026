 ...
 */
export const createManager = functions.https.onCall(async (data: CreateManagerRequest, context): Promise<CreateManagerResponse> => {
  try {
    ...
    if (data.branchCodes && data.branchCodes.length > 0) {
      for (const bCode of data.branchCodes) {
        if (bCode === data.code) continue; // Skip master PIN
    ...
    if (data.branchCodes) {
      for (const bCode of data.branchCodes) {
        if (bCode === data.code) continue; // Skip if same as master PIN
    ...
    if (!data.isDemo) {
      try {
        ...
        const subscriptionPricePerBranch = settings?.defaultSubscriptionPrice || 1000;
        ...
        let discountRate = 0;
        if (subscriptionDuration === 2 && settings?.twoYearDiscountRate) {
        ...
        try {
          ...
          vouchersSnap.docs.forEach(d => {
            const n = d.data()?.voucherNumber;
            if (typeof n === 'number' && n >= voucherNumber) voucherNumber = n + 1;
          });
        } catch (_) { /* keep 1 */ }
        ...
        try {
          ...
          invSnap.docs.forEach(d => {
            ...
            if (typeof n === 'number' && n >= invoiceNumber) invoiceNumber = n + 1;
          });
        } catch (_) { /* keep 1 */ }
